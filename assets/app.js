...
const SECTION_DEFS = [
  ...
  { id: "chips", label: "芯片", short: "芯片", description: "Nvidia/TSMC/Intel/AMD、半导体、算力与数据中心" },
  { id: "hardware", label: "硬件", short: "硬件", description: "手机、PC、穿戴、消费电子与自动驾驶" },
  { id: "robots", label: "机器人", short: "机器人", description: "人形机器人、具身智能、四足与灵巧手" },
  { id: "devtools", label: "软件", short: "软件", description: "开发工具、开源项目、云与开发者实践" },
  { id: "industry", label: "行业", short: "行业", description: "公司战略、融资收购、监管与产业变化" },
...
function itemLabelTone(item) {
  ...
  if (label === "model_release" || label === "ai_model" || label === "agent_workflow") return "models";
  if (label === "chip_semi" || label === "infra_compute") return "chips";
  if (label === "consumer_hw") return "hardware";
  if (label === "robotics") return "robots";
  if (label === "developer_tool" || label === "developer_tooling" || label === "dev_tool" || label === "software_saas") return "devtools";
  if (label === "industry_business") return "industry";
  if (label === "ai_product_update") return "models";
  return "default";
}
...
function labelText(item) {
  ...
  return labels[item.ai_label] || item.ai_label || "科技信号";
}
...
const AI_LABEL_SECTION_MAP = {
  ...
  ai_model: "models",
  ai_product: "models",
  ai_product_update: "models",
  agent_workflow: "models",
  chip_semi: "chips",
  infra_compute: "chips",
  consumer_hw: "hardware",
  robotics: "robots",
  dev_tool: "devtools",
  developer_tool: "devtools",
  developer_tooling: "devtools",
  software_saas: "devtools",
  industry_business: "industry",
};

const SECTION_FALLBACK_RULES = [
  ["chips", [
    /nvidia|tsmc|intel|amd|semiconductor|gpu|npu|cuda|fab|hbm|euv|asik|晶圆|光刻|台积电|英伟达|英特尔|芯片|半导体|算力|数据中心|服务器|集群/,
  ]],
  ["robots", [
    /robot|robotics|humanoid|quadruped|embodied|灵巧手|宇树|figure|机器人|具身|人形|四足/,
  ]],
  ["hardware", [
    /iphone|ipad|macbook|pixel|galaxy|playstation|xbox|switch|nintendo|samsung|apple|手机|平板|笔记本|智能眼镜|可穿戴|无人机|卫星|自动驾驶|robotaxi|self-driving|autonomous|drones/,
  ]],
  ["models", [
    /gpt[-\s]?\d|claude|gemini|grok|llama|qwen|deepseek|mistral|kimi\s?k\d|glm|gemma|模型|model|weights|权重|多模态|视频生成|diffusion|sora|seedance|llm|大模型|人工智能|机器学习|agent|智能体|mcp|openai|anthropic/,
  ]],
  ["devtools", [
    /github|cursor|codex|copilot|openrouter|api|sdk|mcp|cli|framework|linux|kernel|android|ios|chrome|kubernetes|docker|aws|azure|postgres|开源|代码|编程|开发者|漏洞|exploit|vulnerability|ransomware|cloud|数据库|软件/,
  ]],
  ["industry", [
    /funding|raised|ipo|acquire|acquisition|lawsuit|regulation|policy|white house|pentagon|salesforce|meta|microsoft|google|字节跳动|阿里|百度|融资|收购|上市|监管|政策|裁员|估值|公司|行业|政府|五角大楼|白宫/,
  ]],
];
